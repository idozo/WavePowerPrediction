...
files = [('17-33/17-33_', 'DL '), ('17-35/17-35_','UL '), ('19-33/19-33_', 'DR '), ('19-35/19-35_', 'UR ')]
def build_input_for_year(year):
    # df = pd.read_csv("data/database_" + str(year) + ".csv")
    # new_rows = []
    #
    # dupcols = []
    ...
    # new_df.to_csv('data/' + str(year) + '_input_loc.csv', index=False)

    for file,prefix in files:
        dupcols = []
        new_rows = []
        for i in range(hours_before, 0, -1):
            dupcols.append(prefix + str(i) + "AGO wavepower")
        df = pd.read_csv("data/" + file + str(year) + ".csv")
        for i in range(hours_before, len(df.index) - hours_ahead):
            pre_data = pd.DataFrame(df, index=list(range(i - hours_before, i)), columns=['wavepower'])
            new_rows.append(pre_data.values.flatten())
        file_df = pd.DataFrame(new_rows, columns=dupcols)
        print('trying', file)
        # new_df = pd.concat([file_df, new_df], axis=1, sort=False)
        file_df.to_csv("data/" + file + str(year) + "_DONE.csv", index=False)
        print('DONE', file)



def check_file(year):
    database = pd.read_csv("data/database_" + str(year) + ".csv")
    input_file = pd.read_csv('data/' + str(year) + '_input.csv')
...
if __name__ == '__main__':
    print("BUILD 2019 FILE", end="\t")
    build_input_for_year(2017)
    print("- DONE", end="\t")
    # check_file(2019)
    # print('- CHECKED')

    # print("BUILD 2018 FILE", end="\t")
    # build_input_for_year(2018)
    # print("- DONE", end="\t")
    # check_file(2018)
    # print('- CHECKED')
